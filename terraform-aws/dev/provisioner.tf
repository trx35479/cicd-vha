...
resource "null_resource" "bootstrapDevInstance" {
  count = "${var.ENABLE_DEV == "true" ? 1 : 0}"
  triggers {
    instance_id = "${element(module.vha-dev-instance.instance_ids, 0)}"
  }

  connection {
    host        = "${element(module.vha-dev-instance.private_ips, 0)}"
...
resource "null_resource" "bootstrapMtxInstance" {
  triggers {
    instance_id = "${element(module.vha-dev-instance-mtx.instance_ids, 0)}"
  }

  connection {
    host        = "${element(module.vha-dev-instance-mtx.private_ips, 0)}"
    password    = "password"
    user        = "deploy"
    type        = "ssh"
  }
  ...
  depends_on = ["null_resource.bootstrapDevInstance"]
}
